...
export default function NavbarNew() {
  const { language, toggleLanguage, sidebarCollapsed, toggleSidebar, setSearchQuery } = useStore();
        ...
        {user ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              >
                {user.avatar ? (
                  <img src={user.avatar} alt={user.name} className="w-[24px] h-[24px] rounded-[3px] object-cover" />
                ) : (
                ...
                <span className="text-[13px] text-[#37352f] dark:text-[#e0e0e0] font-medium">{user.name}</span>
              </button>
            ...
            {showAvatarMenu && (
              <div className="absolute right-[160px] top-[50px] w-[250px] bg-white dark:bg-[#2f2f2f] border border-[#e0e0e0] dark:border-[#3f3f3f] rounded-[6px] shadow-lg py-2 z-50">
                <p className="px-3 py-1 text-[11px] text-[#9b9a97] uppercase">{t.changeAvatar}</p>
          ...
          <>
            ...
            <Link
              href="/auth"
              className="px-3 py-1.5 text-[13px] bg-gradient-to-r from-[#2383E2] to-[#1a6fc4] text-white rounded-[4px] hover:shadow-lg hover:shadow-[#2383E2]/30 transition-all font-medium"
            >
              {t.signUp}
            </Link>
