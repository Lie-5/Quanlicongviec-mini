...
export default function Sidebar({ language = "en" }: SidebarProps) {
  const pathname = usePathname();
  const { sidebarCollapsed, language: storeLang } = useStore();
  ...
  const bottomItems = [
    ...
    {
      id: "logout",
      label: activeLang === "vi" ? "Đăng xuất" : "Logout",
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <aside className="fixed left-0 top-0 bottom-0 w-64 bg-white border-r border-gray-200 z-40 flex flex-col">
      ...
      <div className="p-5 border-b border-gray-100">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-purple-600 to-blue-500 rounded-xl flex items-center justify-center shadow-md shadow-purple-500/20">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group ${
                  isActive
                    ? "bg-gradient-to-r from-purple-600 to-blue-500 text-white shadow-md shadow-purple-500/25"
        ...
        <nav className="px-3 space-y-1">
          {bottomItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={isActive ? "text-gray-600" : isLogout ? "text-gray-400 group-hover:text-red-500" : "text-gray-400 group-hover:text-gray-600"}>
                  {item.icon}
                </span>
                <span className="text-sm font-medium">{item.label}</span>
              </Link>
